...
4. Вычисли total_damaged_area_m2 — суммарную площадь повреждения в м², УЧИТЫВАЯ ПЕРЕКРЫТИЯ между фото (если два фото показывают одну и ту же повреждённую область с разных ракурсов — не складывай дважды). Это твоя независимая оценка по фото
...
export function parseClaudeResponse(text: string): import("@/types").ClaudeOutput {
  ...
  const aiTotal =
    typeof obj.total_damaged_area_m2 === "number" && obj.total_damaged_area_m2 > 0
      ? obj.total_damaged_area_m2
      : photos.reduce((acc, p) => acc + (p.area_estimate_m2 || 0), 0);
  ...
  return {
    ...
    total_damaged_area_m2: Math.round(aiTotal * 100) / 100,
  };
}
