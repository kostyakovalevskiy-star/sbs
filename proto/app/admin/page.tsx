...
export default function AdminDashboard() {
  ...
  return (
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-5">
        <h1 className="font-display text-2xl sm:text-3xl font-bold text-gray-900 px-1">Дашборд</h1>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-3">
              {[
                ...
                <div key={i} className="bg-white rounded-3xl p-5 sm:p-6">
                  <p className="text-xs text-gray-400 font-medium">{m.label}</p>
                  <p className="font-display text-2xl sm:text-3xl font-bold text-gray-900 mt-2">{m.value}</p>
            ...
            {Object.keys(damageCount).length > 0 && (
              <div className="bg-white rounded-3xl p-5 sm:p-6">
                <h2 className="font-display text-lg font-bold text-gray-900 mb-5">Типы повреждений</h2>
                <div className="space-y-3">
                  {Object.entries(damageCount)
                    .sort((a, b) => b[1] - a[1])
                    .map(([cls, count]) => (
                      <div key={cls} className="flex items-center gap-3">
                        <span className="text-sm text-gray-600 w-40 shrink-0">{DAMAGE_LABELS[cls] ?? cls}</span>
                        <div className="flex-1 h-3 bg-gray-100 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-[#21A038] rounded-full transition-all"
